...
function upload($name2, $type = 'img')
{
    if (isset($_FILES[$name2]["name"]) && $_FILES[$name2]["name"] != '') {
        ...
        $target_dir = $_SERVER["DOCUMENT_ROOT"]."/fundraiser/backoffice/uploads/";
...
function limit_text($content, $max  = 10)
{
    $text   = $content;
    if (str_word_count($text, 0) > $max) {
        $words = str_word_count($text, 2);
        $pos   = array_keys($words);
        $text  = substr($text, 0, $pos[$max]) . '...';
    }
    return $text;
}
